"""
## Extreme fire weather days from GEFF
...
dir_in = dir_base / "03_fire_weather/00_raw_geff/"
dir_out = dir_base / "03_fire_weather/01_preprocessed/"

start_year = 1986
end_year = 2017
param = 'bi'    # choose one of ['bi', 'fwi', 'fdi']
fn_out = f'geff_{param}'
...
for i, year_dir in enumerate(year_dir_list):
    fp_list = fp_list + list(Path.glob(year_dir, '*nc'))
len(fp_list)
...
for i, fp in enumerate(fp_list):
    ...
    if i == 0: # setup
        ...
        min_arr = np.round(np.ma.filled(np.squeeze(fh.variables[param][:]).astype(float), fill_value=np.NaN),1)
        max_arr = min_arr.copy()
    else:
        fwi = np.round(np.ma.filled(np.squeeze(fh.variables[param][:]).astype(float), fill_value=np.NaN),1)
...
utils.output_2d_nc(mygrid=min_arr, mylats=lat, mylons=lon,
                       fp_out=str(dir_out / "{0}_minimum.nc".format(fn_out)), do_zlib=1)
utils.output_2d_nc(mygrid=mid_arr, mylats=lat, mylons=lon,
                       fp_out=str(dir_out / "{0}_midpoint.nc".format(fn_out)), do_zlib=1)
utils.output_2d_nc(mygrid=max_arr, mylats=lat, mylons=lon,
                       fp_out=str(dir_out / "{0}_maximum.nc".format(fn_out)), do_zlib=1)
...
for i, year_dir in enumerate(year_dir_list):
    ...
    fp_list = list(Path.glob(year_dir, '*.nc'))
    ...
    if len(fp_list) > 0:
        utils.output_2d_nc(mygrid=fwi_days, mylats=lat, mylons=lon,
                               fp_out=str(dir_out / "{0}_{1}.nc".format(fn_out, year)), do_zlib=1)
