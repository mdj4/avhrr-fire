"""
## Extreme fire weather days from GFWED - CPC data
...
dir_in = dir_base / "03_fire_weather/00_raw_cpc/"
...
for i, fp in enumerate(fp_list):
    ...
    if i == 0: # setup
        ...
        min_arr = np.round(np.ma.filled(np.squeeze(fh.variables['CPC_FWI'][:]), fill_value=np.NaN),1)
        max_arr = min_arr.copy()
    else:
        fwi = np.round(np.ma.filled(np.squeeze(fh.variables['CPC_FWI'][:]), fill_value=np.NaN),1)
...
utils.output_2d_nc(mygrid=min_arr, mylats=lat, mylons=lon,
                   fp_out=str(dir_out / "fwi_cpc_minimum.nc"), do_zlib=1)
utils.output_2d_nc(mygrid=mid_arr, mylats=lat, mylons=lon,
                   fp_out=str(dir_out / "fwi_cpc_midpoint.nc"), do_zlib=1)
utils.output_2d_nc(mygrid=max_arr, mylats=lat, mylons=lon,
                  fp_out=str(dir_out /"fwi_cpc_maximum.nc"), do_zlib=1)
...
for i, year_dir in enumerate(year_dir_list):
    ...
    if len(fp_list) > 0:
        utils.output_2d_nc(mygrid=fwi_days, mylats=lat, mylons=lon,
                           fp_out=str(dir_out / f"fwi_days_cpc_{year}.nc"), do_zlib=1)
